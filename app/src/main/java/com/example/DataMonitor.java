...
public class DataMonitor extends FragmentActivity implements OnClickListener {
	...
	private void SelectFragment(int Index) {
		...
		//transaction   处理Fragment的操作的集合，包含add(),remove();replace()等操作
		...
		transaction.commit();
		/*调用commit()方法应用上述操作
		* 可以在commit()之前调用addToBackStack()方法加入back stack（返回堆栈），
		* 用户按返回键时回到上一个Fragment状态*/
	...
	public void RefreshButtonStatus(){//按钮状态，选中黑色，否则灰色
	...
	public void ControlClick(View v) {
		switch (v.getId()) {
			case R.id.button0:
				if (selected[0]==false) return;				//是否选中输出
				iCurrentGroup=0;
	...
	public Handler drawlineHandler = new Handler()
	{
		public void handleMessage(Message msg)
		{
